...
const getEvents = async () => {
  ...
  if (token) {
    removeQuery();
    const url =
      "https://0n2w0xz9h3.execute-api.us-west-2.amazonaws.com/dev/api/get-events/" +
      token;
  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const results = await axios.get(
        "https://0n2w0xz9h3.execute-api.us-west-2.amazonaws.com/dev/api/get-auth-url"
      );
...
const getToken = async (code) => {
  ...
  const { access_token } = await fetch(
    `https://0n2w0xz9h3.execute-api.us-west-2.amazonaws.com/dev/api/token/${encodeCode}`
  )
    .then((res) => {
      return res.json();
    })
